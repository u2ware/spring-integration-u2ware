...
public class BacnetSlave implements Runnable , InitializingBean, DisposableBean{
	...
	public static void main(String[] args) throws Exception{
		...
		int port = IpNetwork.DEFAULT_PORT;//47808
		try{
	...
	public void afterPropertiesSet() throws Exception {
		Assert.notNull(localPort, "localPort must not be null.");
		Assert.notNull(localInstanceNumber, "instanceNumber must not be null.");
		new Thread(this).start();
	}

	public void run() {
		try{
	        localDevice = new LocalDevice(localInstanceNumber, new DefaultTransport(new IpNetwork("127.0.0.1", localPort)));
    ...
    private BACnetObject addAnalogInput(LocalDevice localDevice, EngineeringUnits units, float value, float covIncrement)
    ...
    private BACnetObject addBinaryInput(LocalDevice localDevice, String name, String inactiveText, String activeText,
    ...
    private BACnetObject addAnalogValue(LocalDevice localDevice, String name, EngineeringUnits units, float value,
